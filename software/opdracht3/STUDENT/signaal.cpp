...
void SignaalVenster::tekenReeksHandler(wxCommandEvent &event)
{
	...
	{
		PuntLijst punten, fftPuntenArg, fftPuntenMag; /* wxArray van wxPoints */
		...
		if (SignaalType::DataBestand != signalChoice)
		{
...
#endif
			{
				...
				case SignaalType::Driehoek: {
					signalValue = ((2 * amplitude) / PI) * asinf(sin(hoek));
					break;
				}
				case SignaalType::Blokgolf: {
					signalValue = sin(hoek) >= 0.0 ? amplitude : -amplitude;
					break;
				}
				...
				/* Vul indien nodig (volgt uit jullie implementatie hier boven) dit gedeelte in met software instructies.
				* Negeer en verwijder het onderstaande bericht als dit niet het geval is.*/
...
*/
		...
		auto *out = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * (signaal.GetCount() / 2 + 1));
		wxASSERT(nullptr != out);
		//in = (double*)malloc(sizeof(double) * signaal.GetCount());
		...
		const auto  plan = fftw_plan_dft_r2c_1d(signaal.GetCount(), &(signaal[0]), out, FFTW_PRESERVE_INPUT+FFTW_ESTIMATE);
		wxASSERT(nullptr != plan);
		fftw_execute(plan);
